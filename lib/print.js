...
const defaultSections = [
  ...
  },
  {
    ...
    headings: (contest, section, topHeaders) => {
      let topHeader = `${contest.office.area}`;
      let topHeaderCount = topHeaders
        ? topHeaders[`${contest.office.area}`].length
        : null;

      return [
        topHeader,
        topHeaderCount > 1 && !contest.office.subArea && !contest.special
          ? 'School Board'
...
function renderResults(section, contests) {
  ...
  // Grouped of contests by top heading.  this allows for some
  // specific formatting with headings
  let topHeaders = _.groupBy(filtered, c => {
    let h = section.headings(c, section);
    return h ? h[0] : 'no-header';
  });
  ...
  filtered = _.orderBy(
    ...
    [
      c =>
        section.headings(c, section, topHeaders)
          ? makeSort(section.headings(c, section, topHeaders)[0])
          : c.sort,
      'asc'
    ],
    [
      c =>
        section.headings(c, section, topHeaders) &&
        section.headings(c, section, topHeaders)[1]
          ? makeSort(section.headings(c, section, topHeaders)[1])
          : '',
      'asc'
    ],
    [
      c =>
        section.headings(c, section, topHeaders) &&
        section.headings(c, section, topHeaders)[2]
          ? makeSort(section.headings(c, section, topHeaders)[2])
          : '',
      'asc'
    ]
  );
  ...
  filtered.forEach(c => {
    // Headings
    let headings = section.headings(c, section, topHeaders);
    if (headings) {
...
function outputHeadings(current, previous) {
  ...
  current.forEach((h, hi) => {
    if (
      ...
      (changedHeader ||
        hi === current.length - 1 ||
        (!previous || (previous[hi] && h !== previous[hi])))
    ) {
      changedHeader = true;
...
function outputResults(contest, section) {
  ...
  contest.results.forEach(r => {
    ...
    // No for questions should have space
    let tag =
      contest.question && r.candidate.last.match(/^no$/i)
        ? 'candidateNo'
        : 'candidate';

    output += `${printTags[tag]}${
