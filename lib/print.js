...
function outputHeadings(current, previous) {
  ...
  current.forEach((h, hi) => {
    if (
      ...
      changedHeader = true;
      output += `${printTags['heading' + (hi + 2)]}${h}\n`;
    }
  });
...
function outputMeta(contest) {
  ...
  if (
    ...
    output += `${printTags.meta}Open seats: ${contest.elect}\n`;
  }
  ...
  output += `${printTags.meta}${contest.reporting ||
    ...
    contest.totalPrecincts ? contest.reporting / contest.totalPrecincts : 0
  )}%)\n`;
...
function outputResults(contest) {
  ...
  contest.results.forEach(r => {
    ...
    }\t${outputName(r.candidate)}${r.incumbent ? ' (i)' : ''}\t${r.votes ||
